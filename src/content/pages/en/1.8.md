...
## Other Changes
- Level files from v1.7.12 and earlier versions will be automatically converted to UTF-8 encoding and have a backup copy automatically created when saved in v1.8. When creating .smws or .smwp files, it is recommended to ensure that all .smwl levels they contain (i.e., levels from older versions) have been saved in v1.8 beforehand, to avoid compatibility issues such as garbled message text caused by encoding differences.
- Redesigned in-game pause: BGM will pause when the game is paused; when pausing during gameplay, hold Yes for 1 second to exit the level, preventing accidental touches.
